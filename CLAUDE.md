...
- `docint/core/ingest/social_linker.py` — Joins a social export's `postings.csv` to its `media.csv` manifest + files (counter-stripped `Media ID`, basename resolution anywhere under the batch tree) and routes each linked file to the right backend: still images through `images_service.py` (CLIP); audio/video by delegating per-file Nextext routing to the shared `media_transcribe.py` engine. Every artifact — image embedding, keyframe, transcript segment — is stamped with the parent posting's `posting_uuid`, which `_attach_posting_group` uses to group a post with all its media at retrieval time. Artifacts additionally carry the posting's own reference fields (`posting_network`/`posting_author`/`posting_timestamp`/`posting_url`/`posting_text`, built via `build_posting_reference_index` from the `TableReader` postings profile) merged *additively* into their `reference_metadata` — a transcript segment keeps `network: nextext`/`type: transcript_segment`. Pre-existing collections need a re-ingest to gain these fields (no payload migration). **Only the basename of `Exported media filename` is ever used**, looked up in an index of the batch tree — that, not a refusal to recurse, is what keeps an absolute or `../` manifest entry from reaching outside the batch, so subdirectories (`dir/photos/`, `dir/videos/`) are safe to search; a basename occurring in two of them is ambiguous and is skipped unless a copy sits beside the manifest. **Exports carrying no media→posting key at all** (Telegram: `Media ID` == `Network ID` == the media's own message id) are joined by album inference — a multi-item post is N consecutive messages recorded as N media rows but one posting, filed under the group's *last* message id, so a row that names no posting attaches to the first one in its channel at or above its own message number. The **timestamp guard is load-bearing, not a refinement** (`SOCIAL_ALBUM_TOLERANCE_S`, default 5s): it is the only thing standing between a partial export and silent mis-attribution, since with the owning posting absent the next one along would be picked purely by message order. Measured on a real export it costs nothing — every true album member sits 0–1s from its posting while the neighbouring post is hours away. The inference runs only after the declared key fails and requires `Posting ID` to *start with* the row's `Author ID`, which a Meta-style `<postingId>_<accountId>` does not — so key-carrying exports can never reach it. `SOCIAL_ALBUM_LINK_ENABLED=false` disables it. **A third shape carries a crawler-minted UUID as `Posting ID`** while the manifest holds the bare network id, so no compared value can ever be equal; two further fallbacks recover those rows, tried after the declared key and after album inference respectively. `build_posting_url_index` reads the network id back out of the posting's own permalink (`…/reel/<id>/`) — an *exact* key, so it carries no flag and no timestamp check, only a collision drop and an 8-digit floor (a shorter path number is a page, not an id). `build_posting_time_index` is the last resort: same `Author` on the same `Network` within `SOCIAL_ALBUM_TOLERANCE_S`, accepted **only when exactly one** posting falls in the window — two candidates is ambiguity, and a coin flip shown to an investigator as provenance is worse than an unlinked row. It keys on `(Network, Author)` rather than `Author ID` because that is what a *media* row carries, and it shares the album knobs (same trade: a key-less row attached on timestamp agreement); the URL key stays on when they are off. Resolution order is key → URL → album → timestamp, each counted separately in the summary log line.
