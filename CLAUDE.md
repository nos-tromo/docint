...
- Frontend lives in `frontend/`. Keep business logic in the API/agents layer. Frontend dev: pnpm; tests: Vitest. The SPA defaults to a light theme (OS-preference driven, toggled via the shared `AppHeader` portal link + tri-state theme switch — `@infra/ui#v0.6.0`); dark remains available via the toggle or an explicit OS dark preference.
