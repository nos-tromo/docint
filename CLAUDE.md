...
- Frontend lives in `frontend/`. Keep business logic in the API/agents layer. Frontend dev: pnpm; tests: Vitest. The SPA defaults to a light theme (OS-preference driven, toggled via the shared `AppHeader` portal link + tri-state theme switch — `@infra/ui#v0.6.2`); dark remains available via the toggle or an explicit OS dark preference. There is exactly one header row (`Shell.tsx`'s `AppHeader`) — no duplicate sidebar headline. It carries the signed-in identity (`user`, from the authenticated `GET /whoami`, undefined while loading/on error) and the release version (`version`, from `GET /version`); neither is echoed anywhere else.
