...
- **Downloads are an icon, not a word** (`components/common/DownloadAction.tsx`): every export in the SPA goes through `DownloadButton` (file built in-page, handed to `downloadText`) or `DownloadLink` (`<a download>`, server-streamed href) — one bordered square carrying `DownloadIcon`, the conventional arrow-into-a-tray. That icon is an **inline SVG** from `components/common/icons.tsx`, drawn as stroked paths on `currentColor` — never a text character such as `⤓`, `↓`, or `▾`, which render from whatever font the browser and OS happen to pick and are therefore different on every machine. In a control that carries no label of its own, the drawing *is* the whole affordance, so it cannot be left to a font. The icon replaces the **verb**, never the **format**: the full phrase ("Export CSV", "Download session sources (ZIP)") survives as the required `label`, which drives both `aria-label` and `title`, and only where several downloads sit side by side (the entity graph's JSON / GraphML / HTML) does each keep its format visible as `children` — three identical icons in a row is a guessing game. Do not add a new bespoke download `<a>`/`<button>`: the link shape hand-copies `@infra/ui`'s `button({variant:'secondary', size:'sm'})` because the cva recipe is typed but not exported at runtime, and that copy lives in exactly one file.
