...
def test_a_companion_hash_resolves_to_the_file_it_was_read_from(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    def scroll(collection_name: str, **_kw: Any) -> tuple[list[Any], None]:
        ...
        if collection_name.endswith("_images"):
            return [SimpleNamespace(payload={"image_id": "frame-1", "media_file_hash": "clip-h", "source_path": str(clip)})], None
        return [], None
