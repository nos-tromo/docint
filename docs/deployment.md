...
- Receives the following environment variables from the compose file (on
  ...
  | `QDRANT_API_KEY` | not set by compose — reaches the container via `env_file` from `.env`; optional; must match data-plane's `QDRANT__SERVICE__API_KEY` |
...
Set both values, then restart data-plane and docint in that order; docint
returns `401` on every Qdrant call in the window between the two restarts,
and must be on a release that supports `QDRANT_API_KEY`.
