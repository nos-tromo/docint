...
| `QDRANT_API_KEY` | *(unset)* | Sent as the `api-key` header on every Qdrant request. Set it to the same value as data-plane's `QDRANT__SERVICE__API_KEY`; leave unset when Qdrant runs without auth. `qdrant-client` warns once at startup that the key travels over plain HTTP — expected on `data-net`, which never leaves the Docker bridge. |
