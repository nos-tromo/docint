...
| `INGEST_PREPROCESS_WORKERS` | `4` | Worker threads in the per-file preprocessing pool that runs PDF layout/OCR, image caption/OCR/CLIP and Nextext transcription across files at once — see [Per-file preprocessing](ingestion.md#per-file-preprocessing). Bound it by what the inference endpoints can serve concurrently. |
