...
| `SOCIAL_TEXT_LINK_ENABLED` | `true` | Final fallback for a media row whose author names no posting — a shared post, where the manifest records the original author while the export's row is the sharer's. The row attaches to the single posting on its network whose text it repeats verbatim; ambiguity and absence both leave it unlinked, and a posting with no text is never indexed. See [Social media exports](ingestion.md#social-media-exports). |
...
| `SUMMARY_ON_INGEST` | `true` | Whether a collection summary rebuild runs automatically at the end of an ingest job. The deployment default only: the ingest screen's **Summary** toggle (and `IngestIn.summary`) overrides it for one run, which is what a long batch nobody will summarize is expected to use, since the rebuild maps the whole collection and not just that run's files. Fail-soft: an exception from the rebuild (e.g. an LLM outage) is caught, logged, and reported to the client as a `warning` SSE event (`"Collection summary generation failed."`) — the ingest job still completes normally and its documents are ingested and retrievable regardless. |
