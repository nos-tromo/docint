...
| `SOCIAL_ALBUM_LINK_ENABLED` | `true` | Attach a social export's media rows that name no known posting to one by timestamp-corroborated inference — album membership, then the author's own timeline — see [Social media exports](ingestion.md#social-media-exports). Exports that carry a working media→posting key never reach these paths, and the exact URL-derived key stays on when this is `false`. |
| `SOCIAL_ALBUM_TOLERANCE_S` | `5.0` | Maximum timestamp disagreement, in seconds, allowed when accepting either inferred link. Clamped to a minimum of `0`. The tight default is deliberate: it is the only guard against silent mis-attribution when the owning posting is missing from a partial export. |
