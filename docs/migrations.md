...
| Image captions in the operator's language (`RESPONSE_LANGUAGE`) | Re-ingest with `IMAGE_CACHE_BY_HASH=false`, or ingest into a fresh collection. A cache hit returns the stored caption without calling the vision model, so an ordinary re-ingest keeps whichever language the images were first captioned in. See [configuration.md](configuration.md#image-ingestion--imageingestionconfig). |
...
| `keyframe_index` / `keyframe_time_sec` on video keyframes | Re-ingest the clip **after** clearing its cached transcript — a cache hit skips the Nextext round-trip, and with it the keyframes. Delete the collection, or ingest into a fresh one. Needs Nextext ≥ v1.11.0 to have any time to record; an older one leaves `keyframe_time_sec` null and everything else works. |
| The media file's name on social keyframes and transcript segments (`source_file`, `media_file_hash`) | Ingest into a **fresh** collection. An ordinary re-ingest cannot fix the segments: the pipeline skips a file hash it already holds, and a cached transcript hashes identically, so the segments are never rewritten. Keyframes alone would be re-stamped. Segments naming the transient `<clip>.nextext.jsonl` need no migration at all — the read side strips that suffix. See [extracts.md](extracts.md). |
