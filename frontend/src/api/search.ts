...
import type { ChunkText, MetadataFilter, SearchField, SearchRequest, SearchResult } from './types'
 ...
 *
 * One lane: `field` picks which payload field the keywords match, the same
 * way it does for `searchCollection`, and is omitted for `'text'` (the
 * backend's own default) rather than sent redundantly. A blank `question`
 * exports the whole filtered collection — the one export the panel itself
 * cannot show on screen, since `useSearch` refuses to run on an empty query.
 * `sessionId` and `markedIds` both mark chunks for the export server-side —
 * a stored chat scope and an unsaved local selection respectively — and are
 * unioned there, so passing both is safe. Callers should still omit
 * `markedIds` once `sessionId` is set, though: the scope it would name is
 * already persisted server-side by then (see the SearchPanel.tsx call
 * site), and a scope has no count cap — only a token budget — so a
 * legitimate selection can run to hundreds of ids, long enough to overflow
 * the gateway's header limits if serialized here too. Use this as the
 * `href` of a download anchor so the browser handles the streaming response
 * natively.
 *
 * @param collection - Caller's logical collection.
 * @param opts.question - Whitespace-separated keywords, or blank to export
 *   the whole filtered collection.
 * @param opts.field - The payload field to match, or omitted for `'text'`.
...
export function searchExportHref(
  ...
  if (opts.field && opts.field !== 'text') params.set('field', opts.field)
